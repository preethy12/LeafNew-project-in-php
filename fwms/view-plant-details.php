...
$query=mysqli_query($con,"insert into tblplantrequests(plantId,fullName,mobileNumber,message,requestNumber) values('$plantid','$fname','$mnumber','$msg','$reqnumber')");
    echo '<script>alert("Request Successfully Sent. Your request number is  "+"'.$reqnumber.'");</script>';
echo "<script type='text/javascript'> document.location = 'plant-available.php'; </script>";
...
<html>
<head>
<title>Leaf Now|| Plants Available</title>
...
tblplant 
left join tblplantrequests  on tblplant.ID=tblplantrequests.plantId
join tbldonor on tblplant.DonorID=tbldonor.ID
 where  tblplant.ID='$vid'");
while ($row=mysqli_fetch_array($ret)) {
?>
		
		  <section id="tables">
          <div class="page-header">
            <h1> Details Of Plants<?php  echo $row['RequestNumber'];?></h1>
          </div>
...
;?></td>
  ...
  <?php if($row['status']==""){ ?>
  <tr>
  	<td colspan="4"><button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">Order Now</button></td>
  </tr>
<?php }  ?>
  </table><?php } ?>
