...
$fitemarray=implode(",",$fitem);
    $query=mysqli_query($con, "update tblplant set plantItems='$fitemarray',StateName='$statename', CityName='$cityname', Description='$description', PickupDate='$pdate', PickupAddress='$padd',ContactPerson='$contactperson',CPMobNumber='$cpmobnum' where ID='$eid'");
    if ($query) {
  
     echo "<script>alert('Donating  detail has been updated successfully');</script>";
  }
  else
    {
...
<!DOCTYPE html>
<head>
<title>Leaf Now  | Plant Updation </title>
...
$('#add').click(function(){
i++;
$('#dynamic_field').append('<tr id="row'+i+'"><td><input type="text" name="fitem[]" placeholder="Enter Plant Items" class="form-control name_list" /></td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');
});
...
<div class="alert alert-info" role="alert">
    ...
    <?php echo $msg;}  ?>
     ...
     <?php
 $cid=$_GET['editid'];
$ret=mysqli_query($con,"select * from tblplant where ID='$cid'");
$cnt=1;
...
?>
                <form class="form-horizontal bucket-form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label class="col-sm-3 control-label">Plants/seeds</label>
                        <div class="col-sm-6">
                            <table class="table table-bordered" id="dynamic_field">
<tr>
<td><input type="text" name="fitem[]" value="<?php  echo $row['plantItems'];?>" class="form-control name_list" /></td>
