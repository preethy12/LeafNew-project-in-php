...
$fitemarray=implode(",",$fitem);
    $query=mysqli_query($con, "insert into tblplant(DonorID,plantId,StateName, CityName, plantItems, Description, PickupDate,PickupAddress,ContactPerson,CPMobNumber,Image) value('$donorid','$fid','$statename','$cityname','$fitemarray','$description','$pdate','$padd','$contactperson','$cpmobnum','$foodpic')");

    if ($query) {
    echo "<script>alert('Plant Detail added successfully.');</script>";
echo "<script type='text/javascript'> document.location = 'add-plant-details.php'; </script>";
  }
  else
    {
...
<!DOCTYPE html>
<head>
<title>Leaf Now  | Add  Detail </title>
...
<tr>
<td><input type="text" name="fitem[]" placeholder="Enter Plant Items" class="form-control name_list" /></td>
